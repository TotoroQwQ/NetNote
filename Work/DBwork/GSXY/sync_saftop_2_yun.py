...
参数示例:
python sync_saftop_2_yun.py "c91b576f671b4875b43ad1899afe9593","20.0.0.66","1433","sa","sa","whxdl","20.0.0.252","3306","root","root","st_admin2","st_attendance_gs","0","",""
...
def sync_department():
    ...
    print('新增部门： {0}'.format(sync_count))

# 检查部门是否存在
def checkDataGrpExist(name,pid):
    global dest_conn,tid
    dest_cursor = dest_conn.cursor()
    # 在云平台找查找是否存在该部门
    sql='select id from base_data_group FORCE INDEX(i_tid_name) where grp_type=1 and name =\''+name+'\' and tenant_id=\''+tid+'\' and parent_id='+str(pid)
    dest_cursor.execute(sql)
    row = dest_cursor.fetchone()
    return row
...
def addDataGrp(orgpid,destPid,grpType):
    ...
    for row in rows:
        n = checkDataGrpExist(row[1],destPid)
        if n is None:
            sql='insert into base_data_group(name,parent_id,grp_type,tenant_id) values(\'{0}\',{1},{2},\'{3}\')'.format(row[1],destPid,1,tid)
            dest_cursor.execute(sql)
...
def checkUserExist(ucode):
    global dest_conn,tid
    dest_cursor = dest_conn.cursor()
    sql='select id from base_user where username = \''+ucode+'\' and tenant_id = \''+tid+'\''
    dest_cursor.execute(sql)
    row = dest_cursor.fetchone()
    return row
...
def sync_user():
    ...
    for row in rows:
        ...
        # 新增用户与部门的关系
        sql='select t1.name from acscon_group t1 inner join acscon_groupmember t2 on t1.id=t2.gid inner join acscon_user t3 on t3.id=t2.uid where t3.usercode=\'{0}\''.format(row[1])
        org_cursor.execute(sql)
        deps = org_cursor.fetchall()
        # 可能存在多个部门
        for dep in deps:
            #先查找部门ID
            sql='select id from base_data_group FORCE INDEX(i_tid_name) where name=\'{0}\' and grp_type=1 and tenant_id=\'{1}\''.format(dep[0],tid)
            dest_cursor.execute(sql)
...
def sync_point():
    ...
    for row in rows:
        ...
        if p is None:
            ...
            id=p[0]
    print('新增考勤点:{0}'.format(sync_count))
...
def sync_att_data():
    ...
    print("开始同步考勤记录 当前月份:{0} 时间:{1} --> {2}".format(cur_month,sync_time_beg,sync_time_end))
    pgindex=1
    pgsize=1000
    ...
    while True:
        ...
        #继续取下一批数据
        pgindex = pgindex+1
    print('新增考勤记录:{0}'.format(sync_count))
