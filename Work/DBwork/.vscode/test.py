...
def databse_init():
    ...
    sql = ("IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[StudentConvert]') AND type in (N'U'))"
           ...
           " ) ON [PRIMARY]"
           " ALTER TABLE [dbo].[StudentConvert] ADD CONSTRAINT [STUID_UID]  DEFAULT (newsequentialid()) FOR [Uid]"
           " END")
...
def sync_class():
    ...
    if len(Class_Id_List) == 0:
        ...
        return
    #查询需要同步的具体数据
    sql = 'SELECT SUBSTR(groupid, 2, LENGTH(groupid)-1),CLASSNAME,teacherid,STARTTIME,ENDTIME FROM sycomk.D_Class_ALL where groupid in ('
...
def sync_student():
    ...
    for stu in stu_data_tuple:
        sql='\
        IF NOT EXSITS (SELECT STUID FROM STUDENTCONVERT WHERE STUID = \'{0}\')\
        BEGIN\
...
def update_student_class():
    ...
    #更新班级数据
    sql='SELECT CLASSNAME FROM CLASSCONVERT WHERE GID NOT IN (SELECT ID FROM ACSCON_GROUP) GETDATE() BETWEEN STARTTIME AND ENDTIME '
    cursor_dest.execute(sql)
    ...
    if len(newclass)==0:
        ...
        for cla in newclass_list:
            sql='insert into acscon_group (stationid,type,name,pid) values (\'{0}\',\'{1}\',\'{2}\',\'{3}\')'.format(stationid,1,cla[0],'(select id from acscon_group where name=\'班级\')')
            cursor_dest.execute(sql)
        cursor_dest.connection.commit()
